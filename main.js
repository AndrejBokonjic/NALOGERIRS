...
const processPDF = (filePath) => {
    return new Promise((resolve, reject) => {
        ...
        // ZA BUILD
        const pythonProcess = spawn('python', [path.join(__dirname, '..', '/NALOGERIRS/backend/pdfReadPlumber.py'), filePath]);
...
const processModelTypeAndPatientName = (filePath) => {
    return new Promise((resolve, reject) => {
        //ZA BUILD
        const pythonProcess = spawn('python', [path.join(__dirname, '..', '/NALOGERIRS/backend/ImeModelaIzPDF in ImePacientaIzPDF/KategorijaPdfInImePacientaMAIN.py'), filePath]);
...
ipcMain.on("send-table-to-butterfly-model", (event, dataToButterflyModel) => {
    const pythonProcess = spawn('python', [join(__dirname,'..' ,'/NALOGERIRS/backend/butterflyModel.py'),
...
ipcMain.on("send-table-to-range-of-motion", (event, dataToRangeOfMotion) => {
    ...
    const pythonProcess = spawn('python', [join(__dirname, '..', '/NALOGERIRS/backend/rangemotionModel.py'),
...
ipcMain.on("send-table-to-head-neck-model", (event, dataToHeadNeckRelocationModel) => {
    ...
    const pythonProcess = spawn('python', [join(__dirname, '..','/NALOGERIRS/backend/headneckModel.py'),
...
ipcMain.on('create-excel', (event) => {
    const pythonProcess = spawn('python', [path.join(__dirname,'..' ,'/NALOGERIRS/backend/Eksel/generateExcel.py')]);
...
ipcMain.on('save-data-to-excel', (event, data) => {
    const pythonProcess = spawn('python', [path.join(__dirname,'..', '/NALOGERIRS/python/Eksel/saveExcelData.py'), JSON.stringify(data)]);
